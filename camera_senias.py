...
class Camera (AsistenteVoz):
    ...
    def ejecutar(self):
        ...
        cambio_palabra = ctk.CTkButton(master=MyFrame3, text="cambiar", height=40, width=250, border_width=0, corner_radius=12, command=lambda: self.change_text())
        cambio_palabra.pack(side=ctk.TOP, pady=(5, 10))

        self.palabra_aleatoria_label = ctk.CTkLabel(MyFrame3, font=("Consolas", 24))
        self.palabra_aleatoria_label.pack(fill=ctk.X, side=ctk.LEFT, expand=ctk.TRUE, padx=(10, 10), pady=(10, 10))
        self.palabra_aleatoria_label.configure(text=requests.get("https://random-word-api.herokuapp.com/word?number=1").json()[0])
        
        self.resultado_label = ctk.CTkLabel(MyFrame3, font=("Consolas", 24))
        self.resultado_label.pack(fill=ctk.X, side=ctk.LEFT, expand=ctk.TRUE, padx=(10, 10), pady=(10, 10))
        ...
        self.canvas_ahorcado = ctk.CTkCanvas(MyFrame4, height=200, width=200)
        self.canvas_ahorcado.pack(fill=ctk.BOTH, side=ctk.LEFT, expand=ctk.TRUE, padx=(10, 10), pady=(10, 10))
